...
def format_standard_to_utc(datetimeStandard: str):
    ...
    '''
    utc_time = date.strptime(
        datetimeStandard, '%m/%d/%Y %I:%M:%S %p').strftime('%Y-%m-%dT%H:%M:%SZ')
    return utc_time
...
def format_utc_to_standard(datetimeUTC: str):
    ...
    '''
    standard_time = date.strptime(
        datetimeUTC, '%Y-%m-%dT%H:%M:%SZ').strftime('%m/%d/%Y %I:%M:%S %p')
